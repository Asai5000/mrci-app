...
export default function PmisDbClient({ initialData }: Props) {
  ...
  const [genericNamesInput, setGenericNamesInput] = useState(""); // 読みやすい入力用
  ...
  function openEdit(drug: PmisDrug) {
    ...
    setIsNew(false);
    let names: string[] = [];
    try { names = JSON.parse(drug.genericNames); } catch { names = []; }
    setGenericNamesInput(names.join("、"));
    setForm({
  ...
  function buildGenericNamesJson(input: string): string {
    const names = input
      .split(/[、,，\n]/)
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  async function handleSave() {
    ...
    const payload: FormState = {
      ...form,
      genericNames: buildGenericNamesJson(genericNamesInput),
    };
    try {
      if (isNew) {
        ...
        setData((prev) => [
          ...prev,
          { ...payload, id: Date.now() }, // 楽観的追加（実IDはリロード後）
        ]);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-4">
      ...
      <div className="space-y-2">
        {filtered.map((drug) => {
          const names = parseNames(drug.genericNames);
          return (
            <div
              key={drug.id}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm mt-1 text-amber-800 bg-amber-50 rounded px-2 py-1">
                    {drug.recommendation}
                  </div>
                </div>
                <div className="flex gap-2 shrink-0">
                  <Button size="sm" variant="outline" onClick={() => openEdit(drug)}>
                    編集
                  </Button>
                  <Button
                    size="sm"
      ...
      <Dialog open={isNew || editTarget !== null} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <Label>代表的な一般名（読点または改行区切り）</Label>
              <Input
                value={genericNamesInput}
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, targetPatients: e.target.value || null }))
                }
                placeholder="認知症患者全般"
              />
            </div>
