...
2. 優先項目: 禁忌・警告・用法及び用量の記載を最優先で確認し、特に用量・投与間隔の記載は見落とさないこと。
3. 除外項目: 「薬物動態」の項（Tmax・t1/2・AUC・蛋白結合率等）からは情報を参照しないこと。
4. 確実性の原則: 確認された公式情報に基づく内容のみ記載すること。情報が不確かな場合や添付文書上の記載が確認できない場合は、推測で答えず rationale に「添付文書上の確認が必要」と明記すること。
5. ハルシネーション防止: 記憶のみに頼った薬剤情報（用量・禁忌・相互作用等）の記載は行わないこと。根拠が不明確な提案は suggestion として出力しないこと。
...
function buildRenalPromptSection(renalData: RenalData): string {
  ...
  lines.push("1. 各薬剤の腎排泄割合を考慮し、eGFR/CrCl値に基づく用量調整の必要性をPMDA添付文書の「用法及び用量」「禁忌」「慎重投与」の記載に基づいて評価すること（薬物動態の項は参照しないこと）");
